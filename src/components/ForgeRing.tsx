...
const STATIONS = [
  {
    ...
    actions: [
      ...
      { icon: "🌐", title: "WEB SCRAPE", desc: "Pull structured knowledge from URLs", href: "/slm-lab?step=2&mode=scrape" },
  ...
  },
  {
    ...
    actions: [
      { icon: "🍿", title: "POPCORN INJECTION", desc: "Densify stock knowledge — zero data needed", href: "/slm-lab?step=4&mode=popcorn" },
      { icon: "🔨", title: "CDPT PIPELINE", desc: "Builder · Red Team · Systems · Frame Breaker · Empath", href: "/slm-lab?step=2&mode=cdpt" },
      { icon: "🧬", title: "COGNITIVE FINGERPRINT", desc: "Generate a reasoning profile for your model", href: "/slm-lab?fingerprint=true" },
      { icon: "🎛️", title: "BIAS WEIGHTS", desc: "Dial in perspective ratios before forging", href: "/slm-lab?step=4&mode=bias" },
    ],
    cta: "IGNITE THE FORGE",
    ctaHref: "/slm-lab?step=2",
    emberColor: "#00E5FF",
  },
  {
    id: "grind",
    ...
    actions: [
      { icon: "👆", title: "SWIPE REVIEW", desc: "Tinder-style approve / reject for training samples", href: "/review" },
      { icon: "🔍", title: "QUALITY SCAN", desc: "Auto-flag low-quality or duplicate pairs", href: "/slm-lab?step=3&mode=scan" },
      { icon: "✏️", title: "MANUAL EDIT", desc: "Rewrite individual samples by hand", href: "/slm-lab?step=3&mode=edit" },
      { icon: "📊", title: "DATASET STATS", desc: "Coverage, balance, and quality metrics", href: "/slm-lab?step=3&mode=stats" },
    ],
    cta: "START SHARPENING",
    ctaHref: "/review",
    emberColor: "#7FFF00",
  },
  {
    ...
    actions: [
      { icon: "📦", title: "EXPORT TRAINING KIT", desc: "JSONL + train.py + config — ready to run", href: "/slm-lab?step=4&mode=export" },
      { icon: "🚀", title: "DEPLOY PIPELINE", desc: "Step-by-step guide to train on your machine", href: "/deploy" },
      { icon: "📱", title: "PHONE DEPLOY", desc: "Ship your model to Android via Capacitor", href: "/deploy/phone" },
      { icon: "🖥️", title: "SELF-HOST SETUP", desc: "Docker + Ollama + Open WebUI — zero cloud", href: "/self-host" },
    ],
    cta: "PACKAGE THE KIT",
    ctaHref: "/slm-lab?step=4",
    emberColor: "#B44FFF",
  },
];
...
// ── STATION PANEL ─────────────────────────────────────────────────────────────
function StationPanel({ station, isActive, direction }: {
          ...
          <div className="fr-panel-inner fr-glow-pulse"
            ...
            <div className="fr-station-header">
              ...
              <div style={{ flex: 1 }}>
                ...
                <div className="fr-station-sub">{station.sub}</div>
              </div>
            </div>

            {/* Body */}
            <div className="fr-station-body">
              <p style={{ fontFamily: "Chakra Petch, sans-serif", fontSize: 14, color: "rgba(255,255,255,0.5)", marginBottom: 24, fontWeight: 300, lineHeight: 1.7 }}>
                {station.description}
              </p>

              {/* Actions */}
              <div className="fr-action-list">
                {station.actions.map((action, i) => (
                  <motion.a key={action.title} href={action.href}
                    className="fr-action-item"
                    style={{ "--stripe": station.accent } as React.CSSProperties}
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.1 + i * 0.07, duration: 0.4 }}
                    onMouseEnter={e => {
                      const el = e.currentTarget as HTMLElement;
                      el.style.borderColor = `${station.accent}30`;
                    }}
                    onMouseLeave={e => {
                      const el = e.currentTarget as HTMLElement;
...
export function ForgeRing() {
  ...
  // Track scroll position to determine active station
  useEffect(() => {
  ...
  return (
    <div className="fr-root">
      ...
      {/* ── EMBERS ── */}
      {STATIONS.map((s, i) => (
        <Embers key={s.id} color={s.emberColor} active={i === activeIndex} />
      ))}

      {/* ── STATION DOTS (left) ── */}
      <div className="fr-station-ring">
        {/* Connector line */}
        <div className="fr-connector" style={{
          top: "calc(50% - 80px)",
          height: 160,
        }} />
        {STATIONS.map((s, i) => (
      ...
      {activeIndex < STATIONS.length - 1 && (
        <motion.div className="fr-scroll-hint"
          ...
          <div className="fr-scroll-arrow" style={{ borderColor: station.accent }} />
          <span className="fr-scroll-label" style={{ color: station.accent }}>NEXT STATION</span>
        </motion.div>
      )}
      ...
      <div ref={scrollRef} className="fr-scroll">
        {STATIONS.map((s, i) => (
          <div key={s.id} className="fr-snap-section">
            <StationPanel station={s} isActive={i === activeIndex} direction={direction} />
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        <span style={{ fontFamily: "Orbitron, monospace", fontWeight: 900, fontSize: 13, letterSpacing: "0.15em", color: "#FAFCFF" }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {STATIONS.map((s, i) => (
            <button key={s.id}
              style={{
                width: i === activeIndex ? 28 : 16, height: 4,
