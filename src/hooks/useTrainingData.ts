...
export function useScrapeForTraining() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (params: { url: string; dataset_id: string; domain_hint?: string; offload_perspective?: string; debate_mode?: boolean; synthesis_mode?: string; pair_count?: number }) => {
