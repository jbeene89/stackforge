...
"""
Soupy Training Kit — ${datasetName}
...
async function downloadTrainingKit(
  ...
  if (fullOffline) {
    ...
    const defaultSystemPrompt = `You are a CDPT-trained assistant built with Soupy. You analyze topics through multiple cognitive lenses — practical building, adversarial red-teaming, systems thinking, frame-breaking contrarianism, and human empathy. Deliver answers with depth and conviction. No hedging, no filler. When you know something, say it directly.`;
    const finalSystemPrompt = systemPrompt || defaultSystemPrompt;
    const modelfile = `# Soupy — ${datasetName}
...
"""
Soupy Offline Inference — ${datasetName}
...
"""
Soupy Batch Inference — Process multiple inputs offline.
...
Generated by Soupy · ${new Date().toISOString()}
...
// ─── Popcorn-Only Bundle (zero data) ─────────────────────────
const POPCORN_ONLY_ID = "__popcorn_only__";

async function downloadPopcornOnlyKit(baseModel: string) {
  const { default: JSZip } = await import("jszip");
  const zip = new JSZip();
  ...
  const injScript = generateInjectionScript(
    ["roots", "trunk", "canopy"], 1.5, allPerspectives,
    baseModel, "llama3.2:1b", "general", defaultWeights
  );
  zip.file("inject.py", injScript);
  zip.file("injection_config.json", JSON.stringify({
    zones: ["roots", "trunk", "canopy"],
  ...
  zip.file("POPCORN_README.md",
    `# CDPT Popcorn Injection - Zero Data Mode\n\n` +
    `## No dataset needed. No cloud. Pure kernel expansion.\n\n` +
    `This kit densifies a base model using ONLY its own internal knowledge.\n` +
    `The model's stock facts are the kernels. CDPT perspectives are the heat.\n\n` +
    `## Quick Start\n\n` +
    `\`\`\`bash\n` +
    `# 1. Install Ollama (https://ollama.com)\n` +
    `# 2. Pull your base model\n` +
    `ollama pull llama3.2:1b\n\n` +
    `# 3. Run popcorn injection (default even heat)\n` +
    `python3 inject.py\n\n` +
    `# 4. The output popcorn_dataset.jsonl is ready for training\n` +
    `#    Feed it into any LoRA training pipeline\n` +
    `\`\`\`\n\n` +
    `## Bias Presets\n\n` +
    `Edit \`injection_config.json\` to shape the model's personality:\n\n` +
    `| Preset | Style | Key Weights |\n` +
    `|--------|-------|-------------|\n` +
    `| Even Heat | Balanced expansion | All at 1x |\n` +
    `| Novelty Seeker | Creative/divergent | Frame Breaker 3x, Gap Fill 2x |\n` +
    ...
    `| Deep Empathy | Human-centered | Empath 3x, Synthesis 2x |\n\n` +
    `## What This Does\n\n` +
    `1. Each perspective "burner" queries the model about its own knowledge\n` +
    `2. Higher weight = more passes = deeper activation of latent connections\n` +
    `3. Output is CDPT-enriched JSONL ready for LoRA fine-tuning\n` +
    `4. The model teaches itself what it already knows but was trained to suppress\n` +
    `5. Zero internet, zero API keys, zero data upload -- ever\n`
  );
  ...
  zip.file("Modelfile", `# Soupy Popcorn Model
...
`);
  ...
  zip.file("README.md",
    `# Popcorn Only Kit -- Zero Data Densification\n\n` +
    `Generated by Soupy on ${new Date().toISOString()}\n\n` +
    `## What's Inside\n\n` +
    `| File | Purpose |\n` +
    `|------|--------|\n` +
    `| \`inject.py\` | Popcorn injection script (runs via Ollama) |\n` +
    `| \`injection_config.json\` | Perspective weights & bias presets |\n` +
    `| \`POPCORN_README.md\` | Detailed guide |\n` +
    `| \`Modelfile\` | Ollama import template (post-training) |\n\n` +
    `## Quick Start\n\n` +
    `\`\`\`bash\n` +
    `ollama pull llama3.2:1b\n` +
    `python3 inject.py\n` +
    `# -> popcorn_dataset.jsonl is your training data\n` +
    `\`\`\`\n\n` +
    `No dataset required. The model IS the data.\n`
  );
...
export default function DeployPipelinePage() {
  ...
  // Pre-select dataset from URL query param (including popcorn-only)
  useEffect(() => {
    const paramId = searchParams.get("dataset");
    if (paramId === POPCORN_ONLY_ID && !selectedDatasetId) {
      setSelectedDatasetId(POPCORN_ONLY_ID);
    } else if (paramId && !selectedDatasetId && datasets?.some((d) => d.id === paramId)) {
  ...
  const { data: samples } = useSamples(isPopcornOnly ? "" : selectedDatasetId);
  const { isStepCompleted, completedCount, totalSteps, toggleStep } = useDeployStatus(
    isPopcornOnly ? undefined : (selectedDatasetId || undefined)
  );
  ...
  const readiness = useMemo(() => {
    if (isPopcornOnly) return 100; // Popcorn needs no data
  ...
  const currentStep = !selectedDatasetId ? 0 : isPopcornOnly ? 2 : readiness < 100 ? 1 : 2;
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="space-y-3">
        ...
        >
          <div className="space-y-3 pt-2">
            <p className="text-xs text-muted-foreground">
              Once deployed, your on-device SLM can process captures from the Soupy Capture
              page. The flow:
            </p>
