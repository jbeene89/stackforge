...
`;
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4 space-y-6 animate-fade-in">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <Label className="text-base">CDPT Root Injection</Label>
          <Badge variant="outline" className="text-[9px] bg-forge-emerald/10 border-forge-emerald/30 text-forge-emerald">🌳 AMPLIFY</Badge>
        </div>
        <p className="text-xs text-muted-foreground -mt-1">
          Instead of removing — <span className="text-primary font-medium">inject your 13-pass cognitive data INTO the base model's existing weights</span>. 
          Soup up the tree from roots to the highest branch.
        </p>
        ...
        {/* CDPT Perspective Injection Toggles */}
        <div className="space-y-2">
          <Label className="text-xs">Perspectives to Inject</Label>
          <p className="text-[10px] text-muted-foreground">Choose which of your 13-pass cognitive data gets woven into the base weights</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
            {([
              ...
              <label key={p.key} className={`flex items-center gap-2 px-2.5 py-2 rounded-md border cursor-pointer transition-all ${
                injectionPerspectives.includes(p.key) 
                  ? "border-primary/40 bg-primary/10" 
                  : "border-border hover:border-primary/20"
              }`}>
                <Switch 
                  checked={injectionPerspectives.includes(p.key)} 
          ...
          <Card className="bg-primary/5 border-primary/20">
            <CardContent className="py-3 space-y-2">
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs font-medium">Injection Summary</p>
              </div>
              <p className="text-xs text-muted-foreground">
                <span className="text-primary font-medium">Zero upload required.</span> This densifies what the base model already knows — reshaping existing weights through your cognitive lens. 
                Targeting {injectionZones.map(z => z === "roots" ? "perception" : z === "trunk" ? "knowledge" : "expression").join(" + ")} layers 
                at <span className="font-mono">{injectionIntensity}x</span> intensity 
                using {injectionPerspectives.length}/9 perspective channels.
                {perspectiveSamples.length > 0 && ` Guided by ${perspectiveSamples.length} CDPT-enriched samples.`}
              </p>
              <p className="text-[10px] text-muted-foreground/80 mt-1">
                The base model is already trained on billions of tokens. Your 13 perspectives don't add new data — they <span className="italic">reorganize what's already there</span> around your thinking patterns. New model, same weights.
              </p>
              <p className="text-[10px] italic opacity-70 mt-1">
                {[
                  "🌳 \"Not uploading data — reorganizing the universe the model already contains. It's like rearranging furniture in an infinite house.\"",
                  "🌳 \"The base model already knows everything. We're just teaching it to think about it the way YOU do.\"",
                  "🌳 \"13 cognitive passes, zero new tokens. Same tree, completely different fruit.\""
                ][Math.floor(Date.now() / 60000) % 3]}
              </p>
            </CardContent>
          </Card>
          ...
          <Button
            ...
            onClick={() => {
              const injScript = generateInjectionScript(
                ...
                baseModel, "llama3.2:1b", dataset.domain || "general"
              );
              const zip = new JSZip();
              const folderName = `${dataset.name.toLowerCase().replace(/\s+/g, "-")}-injection-kit`;
              const folder = zip.folder(folderName)!;
              ...
              toast.success("Injection kit downloading! No training data needed 🌳");
            }}
          >
            <Zap className="h-4 w-4 mr-2" /> Download Injection-Only Kit
