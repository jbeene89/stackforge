        ...
        {injectionZones.length > 0 && (
          <Card className="bg-primary/5 border-primary/20">
            <CardContent className="py-3 space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Your {perspectiveSamples.length > 0 ? perspectiveSamples.length : approvedSamples.length} CDPT-enriched samples will generate <span className="text-primary font-medium">amplification training data</span> that 
                reinforces {injectionZones.map(z => z === "roots" ? "perception" : z === "trunk" ? "knowledge" : "expression").join(" + ")} layers 
              ...
              <p className="text-[10px] italic opacity-70">
                {[
                  "🌳 \"Not pruning the tree — pouring rocket fuel on the roots and watching it grow through the ceiling.\"",
                  "🌳 \"Every layer gets seasoned. The base model already knows things — now it'll know them YOUR way.\"",
                  "🌳 \"13 passes of cognitive data, injected from roots to canopy. The base model is about to have opinions.\""
                ][Math.floor(Date.now() / 60000) % 3]}
              </p>
            </CardContent>
          </Card>
