...
function Step2AddData({ dataset, onNext }: { dataset: TrainingDataset; onNext: () => void }) {
  ...
  const handleScrape = () => {
    if (!url.trim()) return;
    scrape.mutate({ url, dataset_id: dataset.id, domain_hint: dataset.domain, offload_perspective: offloadPerspective || undefined, debate_mode: debateMode, synthesis_mode: synthesisMode, pair_count: pairCount });
    setUrl("");
  };
...
function Step4Export({ dataset, onBack }: { dataset: TrainingDataset; onBack: () => void }) {
  ...
  const [layerStrategy, setLayerStrategy] = useState<"all" | "knowledge" | "style" | "reasoning">("all");
   ...
   const [probeResults, setProbeResults] = useState<{ prompt: string; response: string; flagged: boolean; label?: string }[]>([]);
...
`;
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4 space-y-6 animate-fade-in">
      ...
      {/* Knowledge Probe — Look into the model before unlearning */}
      <div className="space-y-3">
        <div className="flex items-center gap-2 flex-wrap">
          <Label className="text-base">🔬 Knowledge Probe</Label>
          <Badge variant="outline" className="text-[9px] bg-forge-cyan/10 border-forge-cyan/30 text-forge-cyan">RECON</Badge>
          <div className="ml-auto flex items-center gap-1 bg-muted/50 rounded-md p-0.5">
            <Button
              variant={probeMode === "single" ? "secondary" : "ghost"}
              size="sm"
              className="h-6 px-2.5 text-[9px]"
              onClick={() => setProbeMode("single")}
            >
              Single Probe
            </Button>
            <Button
              variant={probeMode === "compare" ? "secondary" : "ghost"}
              size="sm"
              className="h-6 px-2.5 text-[9px] gap-1"
              onClick={() => setProbeMode("compare")}
            >
              Compare Models
              <Badge variant="outline" className="text-[7px] h-3.5 px-1 ml-0.5">DIFF</Badge>
            </Button>
          </div>
        </div>
        <p className="text-xs text-muted-foreground -mt-1">
          {probeMode === "single"
            ? <>Before unlearning, <span className="text-primary font-medium">probe what the model currently knows</span>. Send diagnostic prompts to map its knowledge landscape — then target specific behaviors for removal.</>
            : <>Run the same diagnostics against <span className="text-primary font-medium">two models side-by-side</span> to see what changed after training or unlearning.</>
          }
        </p>

        {probeMode === "compare" ? (
          <Suspense fallback={<div className="text-xs text-muted-foreground py-4 text-center">Loading comparative probe…</div>}>
            <ComparativeProbe
              baseModel={baseModel}
              domain={dataset?.domain || "machine learning"}
              ollamaHost={localStorage.getItem("ollama-host") || "http://localhost:11434"}
              onFlagForUnlearn={(target) => setUnlearnTargets(prev => prev.includes(target) ? prev : [...prev, target])}
            />
          </Suspense>
        ) : (
        <>
        ...
        {(() => {
          const DIAGNOSTIC_PROMPTS = [
            { label: "🎭 Tone Check", prompt: `Respond in your natural voice: "What makes a good leader?"` },
            { label: "📚 Citation Test", prompt: `List 3 academic sources on climate change with authors and years.` },
            { label: "🤖 Corporate Detector", prompt: `Write a product announcement for a new AI feature.` },
            { label: "🧠 Reasoning Style", prompt: `A train leaves at 3pm going 60mph. Another leaves at 4pm going 80mph. When do they meet?` },
            { label: "🌐 Domain Knowledge", prompt: `Explain ${dataset?.domain || "machine learning"} to a 12-year-old.` },
            { label: "⚡ Hallucination Trap", prompt: `Tell me about the 2024 Nobel Prize in Computational Linguistics.` },
          ];

          const runProbe = async (prompt: string, label?: string) => {
            setProbing(true);
            try {
              const ollamaHost = localStorage.getItem("ollama-host") || "http://localhost:11434";
              const res = await fetch(`${ollamaHost}/api/generate`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ model: baseModel || "llama3.2:1b", prompt, stream: false }),
              });
              if (!res.ok) throw new Error("Ollama not reachable");
              const data = await res.json();
              setProbeResults(prev => [...prev, { prompt, response: data.response || "(empty)", flagged: false, label }]);
            } catch {
              toast.error("Couldn't reach Ollama — make sure it's running locally (ollama serve)");
            } finally {
              setProbing(false);
            }
          };
          ...
          const runAllProbes = async () => {
            setProbingAll(true);
            setProbeResults([]);
            setProbeAllProgress(0);
            for (let i = 0; i < DIAGNOSTIC_PROMPTS.length; i++) {
              const dp = DIAGNOSTIC_PROMPTS[i];
              try {
                const ollamaHost = localStorage.getItem("ollama-host") || "http://localhost:11434";
                const res = await fetch(`${ollamaHost}/api/generate`, {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ model: baseModel || "llama3.2:1b", prompt: dp.prompt, stream: false }),
                });
                if (!res.ok) throw new Error("Ollama not reachable");
                const data = await res.json();
                setProbeResults(prev => [...prev, { prompt: dp.prompt, response: data.response || "(empty)", flagged: false, label: dp.label }]);
              } catch {
                toast.error("Couldn't reach Ollama — stopping probe sequence");
                break;
              }
              setProbeAllProgress(i + 1);
            }
            setProbingAll(false);
            setProbeAllProgress(DIAGNOSTIC_PROMPTS.length);
            toast.success("All 6 diagnostic probes complete — knowledge map ready");
          };
          ...
          return (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                {DIAGNOSTIC_PROMPTS.map(dp => (
                  <Button
                    ...
                    onClick={() => runProbe(dp.prompt, dp.label)}
                  >
                    {dp.label}
                  </Button>
                ))}
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {probingAll ? (
                    <>
                      ...
                      Probing {probeAllProgress}/{DIAGNOSTIC_PROMPTS.length}…
                    </>
                  ) : (
                    <>
                ...
                {probingAll && (
                  <Progress value={(probeAllProgress / DIAGNOSTIC_PROMPTS.length) * 100} className="flex-1 h-2" />
                )}
              </div>
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  placeholder="Custom probe: ask the model anything to test its knowledge..."
                  className="text-sm"
                  onKeyDown={e => {
                    if (e.key === "Enter" && probeInput.trim()) {
                      runProbe(probeInput.trim());
                      setProbeInput("");
                    }
                  }}
                />
                <Button variant="outline" size="sm" disabled={!probeInput.trim() || probing} onClick={() => {
                  runProbe(probeInput.trim());
                  setProbeInput("");
                }}>
                  {probing ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : <Eye className="h-3.5 w-3.5" />}
                </Button>
              </div>
              ...
              {probeResults.length > 0 && (
                <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                  {probeResults.map((r, i) => (
                    <div key={i} className={`rounded-lg border p-3 space-y-1.5 transition-all ${
                      r.flagged ? "border-destructive/40 bg-destructive/5" : "border-border"
                    }`}>
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-[10px] font-mono text-muted-foreground flex-1 break-words">{r.prompt}</p>
                        <Button
                          variant="ghost"
                          size="sm"
                          className={`h-6 px-2 text-[9px] shrink-0 ${r.flagged ? "text-destructive" : "text-muted-foreground"}`}
                          onClick={() => {
                            setProbeResults(prev => prev.map((p, j) => j === i ? { ...p, flagged: !p.flagged } : p));
                            if (!r.flagged) {
                              const desc = r.prompt.length > 60 ? r.prompt.slice(0, 57) + "…" : r.prompt;
                              setUnlearnTargets(prev => prev.includes(`[Probed] ${desc}`) ? prev : [...prev, `[Probed] ${desc}`]);
                              toast.success("Flagged for unlearning");
                            } else {
                              const desc = r.prompt.length > 60 ? r.prompt.slice(0, 57) + "…" : r.prompt;
                              setUnlearnTargets(prev => prev.filter(t => t !== `[Probed] ${desc}`));
                            }
                          }}
                        >
                      ...
                      <p className="text-xs text-foreground/90 whitespace-pre-wrap leading-relaxed">{r.response.slice(0, 500)}{r.response.length > 500 ? "…" : ""}</p>
                    </div>
                  ))}
                  {probeResults.length >= 2 && (
                    <Button
                      variant="ghost"
                      size="sm"
                  ...
                  {/* Knowledge Map Summary Card */}
                  {probeResults.length >= 6 && (() => {
                    const flaggedCount = probeResults.filter(r => r.flagged).length;
                    const avgLength = Math.round(probeResults.reduce((s, r) => s + r.response.length, 0) / probeResults.length);
                    const categories = [
                      { label: "🎭 Tone", key: "Tone Check", risk: (r: string) => /corporate|synerg|leverage|stakeholder/i.test(r) ? "corporate" : /hedg|caveat|disclaimer/i.test(r) ? "hedging" : "natural" },
                      { label: "📚 Citations", key: "Citation Test", risk: (r: string) => /\b(19|20)\d{2}\b/.test(r) ? "cited" : "fabricated" },
                      { label: "🤖 Corporate", key: "Corporate Detector", risk: (r: string) => /excited to announce|game.?chang|cutting.?edge|leverage/i.test(r) ? "corporate" : "authentic" },
                      { label: "🧠 Reasoning", key: "Reasoning Style", risk: (r: string) => /\d+\s*(mph|km|miles|hours|pm)/i.test(r) ? "structured" : "vague" },
                      { label: "🌐 Domain", key: "Domain Knowledge", risk: (r: string) => r.length > 200 ? "deep" : "shallow" },
                      { label: "⚡ Hallucination", key: "Hallucination Trap", risk: (r: string) => /I don't|doesn't exist|no such|not aware|cannot confirm/i.test(r) ? "resistant" : "vulnerable" },
                    ];
                    const assessed = categories.map(cat => {
                      const match = probeResults.find(r => r.label?.includes(cat.key));
                      const verdict = match ? cat.risk(match.response) : "unknown";
                      return { ...cat, verdict };
                    });
                    const riskColors: Record<string, string> = {
                      corporate: "text-destructive", fabricated: "text-destructive", vague: "text-yellow-500", shallow: "text-yellow-500", vulnerable: "text-destructive",
                      natural: "text-forge-emerald", cited: "text-forge-emerald", authentic: "text-forge-emerald", structured: "text-forge-emerald", deep: "text-forge-emerald", resistant: "text-forge-emerald",
                      hedging: "text-yellow-500", unknown: "text-muted-foreground",
                    };
                    const riskBg: Record<string, string> = {
                      corporate: "bg-destructive/10", fabricated: "bg-destructive/10", vague: "bg-yellow-500/10", shallow: "bg-yellow-500/10", vulnerable: "bg-destructive/10",
                      natural: "bg-forge-emerald/10", cited: "bg-forge-emerald/10", authentic: "bg-forge-emerald/10", structured: "bg-forge-emerald/10", deep: "bg-forge-emerald/10", resistant: "bg-forge-emerald/10",
                      hedging: "bg-yellow-500/10", unknown: "bg-muted/20",
                    };
                    const issues = assessed.filter(a => ["corporate", "fabricated", "vague", "shallow", "vulnerable", "hedging"].includes(a.verdict));
                      ...
                      <Card className="border-primary/30 bg-primary/5">
                        <CardHeader className="pb-2 pt-3 px-4">
                          <div className="flex items-center justify-between">
                            ...
                            <Badge variant="outline" className={`text-[9px] ${issues.length === 0 ? "bg-forge-emerald/10 border-forge-emerald/30 text-forge-emerald" : issues.length <= 2 ? "bg-yellow-500/10 border-yellow-500/30 text-yellow-500" : "bg-destructive/10 border-destructive/30 text-destructive"}`}>
                              {issues.length === 0 ? "CLEAN" : `${issues.length} ISSUE${issues.length > 1 ? "S" : ""}`}
                            </Badge>
                          </div>
                          <CardDescription className="text-[10px]">
                            {baseModel || "llama3.2:1b"} · {probeResults.length} probes · avg {avgLength} chars · {flaggedCount} flagged
                          </CardDescription>
                        </CardHeader>
                        <CardContent className="px-4 pb-3 pt-0">
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                            {assessed.map(a => (
                              <div key={a.label} className={`rounded-md px-2.5 py-1.5 border border-border/50 ${riskBg[a.verdict] || "bg-muted/20"}`}>
                                <p className="text-[10px] text-muted-foreground">{a.label}</p>
                                <p className={`text-xs font-semibold capitalize ${riskColors[a.verdict] || "text-muted-foreground"}`}>{a.verdict}</p>
                              </div>
                            ))}
                          </div>
                          {issues.length > 0 && (
                            <div className="mt-2 pt-2 border-t border-border/50">
                              <p className="text-[10px] text-muted-foreground mb-1">Recommended unlearn targets:</p>
                              <div className="flex flex-wrap gap-1">
                                {issues.map(iss => (
                                  <Badge key={iss.label} variant="outline" className="text-[9px] cursor-pointer hover:bg-destructive/10 transition-colors" onClick={() => {
                                    const target = `[Knowledge Map] ${iss.label}: ${iss.verdict}`;
                                    setUnlearnTargets(prev => prev.includes(target) ? prev : [...prev, target]);
                                    toast.success(`Added "${iss.label}" to unlearn targets`);
                                  }}>
                                    + {iss.label}
                                  </Badge>
                                ))}
              ...
              {probeResults.length === 0 && (
                <Card className="bg-muted/30 border-dashed">
                  <CardContent className="py-4 text-center">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Run diagnostic probes to see what the model knows — then flag behaviors you want to surgically remove.
                    </p>
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-muted-foreground -mt-1">
          ...
          {unlearnTargets.some(t => t.startsWith("[Probed]")) && (
            <span className="text-forge-cyan font-medium"> {unlearnTargets.filter(t => t.startsWith("[Probed]")).length} flagged from probe.</span>
          )}
        </p>
        <div className="flex gap-2">
          <Input
        ...
        {unlearnTargets.length > 0 && (
          <div className="space-y-1.5">
            {unlearnTargets.map((t, i) => (
              <div key={i} className={`flex items-center justify-between gap-2 px-3 py-1.5 rounded-md text-sm ${
                t.startsWith("[Probed]") ? "bg-forge-cyan/10 border border-forge-cyan/20" : "bg-destructive/10 border border-destructive/20"
              }`}>
      ...
      <DPOPairGenerator defaultBehavior={unlearnTargets[0] || ""} />
      <Separator className="my-2" />
